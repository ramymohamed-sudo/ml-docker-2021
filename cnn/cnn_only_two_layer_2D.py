...
def model_builder(hp, filters, units, n_FC, activation_rnn, activation_dense, lr, dropout, MODEL=MODEL):
    ...
    if MODEL == 'MODULAR':
        ...
        # # default strides for Conv1D is 1 - for overlap
        # model.add(MaxPooling1D(pool_size=2, strides=2, padding='same'))
        model.add(Conv1D(filters=filters, kernel_size=2,
...
def model_tuner(hp):
    filters = hp.Int("filters", min_value=12, max_value=32, step=6)
    ...
    model = model_builder(hp, filters, units, n_FC, activation_rnn, activation_dense, lr, dropout)
...
for i in range(len(train_file)):
    ...
    # keras.utils.plot_model(model, "CONV_ONLY.png")
    # plt.show()
    # for layer in model.layers:
    # print(layer.output_shape)

    print(f" 0000000000000000000 end of file number {i+1} for Convolutional ONLY 2 layers 0000000000000000000")
    print(f" 0000000000000000000 end of file number {i+1} for Convolutional ONLY 2 layers 0000000000000000000")
